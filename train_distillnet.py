...
def do_training_and_physicstest_DistillNet(
    filedir: str,
    savedir: str,
    device: str = "cuda:0",
    run_number: int = 0,
    numtests: int = 0,
    wgt: float = 3,
    is_ensembletest: bool = False,
):
    """
    Train and evaluate Distillnet on given samples. Possible either once by executing main() in this script or as
    ensembletest, utilizing the run_distillensemble script.
    """
    ...
    model, criterion, optimizer, train_loader, test_loader, test, input_size = nn_setup(
        nn_inputdata,
        device,
        hparams["batch_size"],
        hparams["maketrain_particles"],
        hparams["train_split"],
        hparams["L1_hsize"],
        hparams["L2_hsize"],
        hparams["n_outputs"],
    ...
    model, losslist, validationloss = do_training(
        model,
        criterion,
        optimizer,
        device,
        ...
        saveinfo,
        weights_highval,
        trainparams["n_epochs"],
        is_dotaylor=is_do_taylor,
    ...
    met_model = load_bestmodel(
        saveinfo,
        modelsavedir,
        trainparams["bestmodel_losstype"],
        device,
        input_size,
        hparams["L1_hsize"],
        hparams["L2_hsize"],
