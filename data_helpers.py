...
def makeratio(val_of_bins_x1, val_of_bins_x2):
    ...
    error = np.divide(val_of_bins_x1 * np.sqrt(val_of_bins_x2) + val_of_bins_x2 * np.sqrt(val_of_bins_x1),
                    np.power(val_of_bins_x2, 2), where=(val_of_bins_x2 != 0))
    return ratio, error
...
def getdata_deposits(filedir: str, sample: str, flist_inputs: list, scalerdir: str, device:str, min_event: int, is_remove_padding: bool = True,
                 is_standard: bool = True, is_min_max_scaler: bool = True, is_standard_scaler: bool = False, is_makeplots: bool = False,is_dtrans: bool= False, is_makeprints: bool = True):
    ...
    with h5py.File(filename, "r") as f:
        ...
        if is_remove_padding:
            print("Removing padded particles.....")
            padmask_px = np.abs(features_reshaped[:, 0]) > 0.000001
            features_nopad, abc_nopad, puppiw_nopad, pu_vec_nopad, nopu_vec_nopad = features_reshaped[padmask_px], abc_truth_reshaped[padmask_px], puppiw_reshaped[padmask_px], pu_vec_reshaped[padmask_px], nopu_vec_resphaed[padmask_px]
            print(f'Shape of input matrix without padding {features_nopad.shape}')
            print(f'Shape of abc truth without padding {abc_nopad.shape}')
        else:
            print("No padding removal")
...
def make_lossplot(losslist: list, validationloss: list, plotdir: str, plotdir_pdf: str, saveinfo: str, timestr:str, is_savefig: bool = False, is_displayplots: bool = False):
...
def make_histoweight(test: list, test_loader, model, device: str, plotdir: str, plotdir_pdf: str, saveinfo: str, timestr:str, is_savefig: bool = False, is_displayplots: bool = False):
    truth = test[1]
    truth = np.concatenate(truth)
    weight_prediction = do_weightpred(test_loader, model, device)
    # weight_prediction=np.squeeze(weight_prediction)
...
def make_metplots(abcMET: list, puppiMET: list, distilMET: list,res_abc: float, res_model: float, res_puppi: float, plotdir: str, plotdir_pdf: str, saveinfo: str, timestr: str, is_savefig: bool = True, is_displayplots: bool = False):

    figure,ax = plt.subplots(2,1,figsize=(8,7),gridspec_kw={'height_ratios': [0.8, 0.2]})
    binsspace=np.arange(0,160,8)
    bins_abc, xb, _ = ax[0].hist(np.clip(abcMET,binsspace[0],binsspace[-1]), bins=binsspace, histtype='step',label=r'$E_\mathrm{T}^{\mathrm{miss}}$ GNN',
                                range=(0, 300), lw=1.7)
    bins_puppi, _, _ = ax[0].hist(np.clip(puppiMET,binsspace[0],binsspace[-1]), bins=xb, histtype='step', label=r'$E_\mathrm{T}^{\mathrm{miss}}$ PUPPI', lw=1.4)
    bins_distil, _, _ = ax[0].hist(np.clip(distilMET,binsspace[0],binsspace[-1]), bins=xb, histtype='step', label=r'$E_\mathrm{T}^{\mathrm{miss}}$ DistillNet', lw=1.7)
    #bins_gen, _, _ = ax[0].hist(np.clip(genMET,binsspace[0],binsspace[-1]), bins=xb, histtype='step', label='MET gen', lw=2.5)
    ...
    bincenter =( 5 + xb[:-1])
    ax[1].errorbar(bincenter,ratio4,yerr=error4,fmt='^',c='tab:orange',ecolor='tab:orange',capsize=5,ms=6,label=f"Ratio PUPPI"+r"and GNN $E_\mathrm{T}^{\mathrm{miss}}$")

    ax[1].errorbar(bincenter,ratio3,yerr=error3,fmt='.k',c='g',ecolor='g',capsize=5,ms=10,label=f"Ratio DistillNet"+r"and GNN $E_\mathrm{T}^{\mathrm{miss}}$")


    ax[1].set_xlabel(r'Missing Transverse Energy $E_\mathrm{T}^{\mathrm{miss}}$ in GeV')

    #ax[0].legend(fancybox=True, framealpha=0.8, loc='best',prop={'size': 18},bbox_to_anchor = (1.02,1),bbox_transform = ax[0].transAxes,borderaxespad =0)
    ...
    #plt.savefig(results_dir+"MET_complete_ratio.pdf",bbox_inches='tight')
    #plt.savefig(results_dir+"MET_complete_ratio",dpi=500,bbox_inches='tight')

def make_histoweight_mod(predictions, truth, puppiw, res_model,res_abc,res_puppi, plotdir, plotdir_pdf, saveinfo, timestr,sample, is_savefig: bool= True, is_displayplots: bool = False):
    predictions = np.concatenate(predictions)
    truth = np.concatenate(truth)
    puppiw = np.concatenate(puppiw)
   # wdict = {'predictions':predictions,
   #          'truth':truth,
   #          'puppiw':puppiw}
   # df = pd.DataFrame.from_dict(wdict)

    def makehistograms(predictions,truth,remove_padding: bool = False):
        bins=20
        ranges=(0,1)
        bin1, xb, xr = plt.hist(truth,bins=bins,range=ranges,label=f"GNN truth weights",histtype="step",lw=2)
        bins_pup,_,_ = plt.hist(puppiw,bins=20,range=(0,1),label="PUPPI weights ",histtype="step",lw=1.7,color='#ff7f0e')
    ...
    bins1,bins2,xb,xr = makehistograms(predictions,truth,True)
    ratiolast = (bins2[-1]/bins1[-1])
    ratiofirst = (bins2[0]/bins1[0])
    plt.plot([], [], ' ', label=f"First bin ratio:"+r"$\,$"+f"{ratiofirst*100:.2f}"+r"$\%$"+
                            f"\nLast bin ratio: {ratiolast*100:.2f}"+r"$\%$"+
                            f"\nResolution GNN: {res_abc:.4f}"+
                            f"\nResolution Model: {res_model:.4f}"+
                            f"\nResolution PUPPI: {res_puppi:.4f}")

    #bins_pup,_,_ = plt.hist(puppiw,bins=20,range=(0,1),label="PUPPI weights ",histtype="step",lw=1.2,color='#ff7f0e')
    ...
    plt.minorticks_on()
    plt.legend(prop={'size': 18},fancybox=True,framealpha=0.8,loc='upper center',bbox_to_anchor = (1.23,1),bbox_transform = plt.gca().transAxes,borderaxespad =0)
    ...
    sample_file_name1 = "wgts"
    if is_savefig:
        plt.savefig(plotdir + sample_file_name1 + saveinfo + sample.replace('.h5', '') + '__time_' + timestr + '.png', dpi=400, bbox_inches='tight')
        plt.savefig(plotdir_pdf + sample_file_name1 + saveinfo + sample.replace('.h5', '') + '__time_' + timestr + '.pdf', bbox_inches='tight')
        #magicdir = '/work/tbrandes/work/Delphes_samples/met_results/'
        #df.to_hdf(magicdir + sample_file_name1 + saveinfo + sample, key='weights', mode='w')  
    if is_displayplots:
        plt.show()
    else:
        plt.clf()
...
def gettraindata_oldnet(filename, flist, standard: bool = True, remove_padding: bool = True, is_makeplots: bool = False):
    ...
    with h5py.File(filename, "r") as f:
        ...
        if is_makeplots:
            ...
            for j in range(0,len(combinedfeatures[0,:])):
                ax = plt.subplot(6, 3, j + 1)
                bins = ax.hist(combinedfeatures[:,j],label=f"var {j}",bins=20,color='b',alpha=0.5)
                ax.set_yscale('log')
                ax.legend()
            plt.show()
        if standard:  # fit data via scipy to later apply standardization
            ...
            dump(
                scaler,
                f"removepadding{remove_padding}" + "std_scaler_reducedfeatures.bin",
                compress=True,
            )
        ...
        if is_makeplots:
            ...
            for j in range(0,len(combinedfeatures_trans[0,:])):
                ax = plt.subplot(6, 3, j + 1)
                bins = ax.hist(combinedfeatures_trans[:,j],label=f"var {j}",bins=20,color='b',alpha=0.5)
                ax.set_yscale('log')
                ax.legend()
            plt.show()
...
def plot_jetresolution(responseabc, responsedistil, responsepuppi, results_dir: str, results_dir_pdf: str, ptcut: int, is_savefig: bool = True, is_corr: bool = False, is_ptcorr: bool = False):
    ...
    #print("Ratio distil better than PUPPI: ",100*resolution(responsedistil)/resolution(responsepuppi_true))
    #print("Ratio abc better than PUPPI: ",100*resolution(responseabc)/resolution(responsepuppi_true))
    ...
    bins_abc, xb, _ = plt.hist(responseabc,bins=binsspace, histtype='step',label=r'$E_\mathrm{Jet,\,reco}$'+f' GNN\nResolution: {r1:.4f}',lw=1.5,range=range)
    bins_puppi, xb, _ = plt.hist(responsepuppi,color='#ff7f0e', bins=binsspace, histtype='step', label=r'$E_\mathrm{Jet,\,reco}$'+f' PUPPI\nResolution: {r4:.4f}', lw=1.5,range=range)
    #bins_puppi, xb, _ = plt.hist(responsepuppi, bins=binsspace, histtype='step', label='Jets PUPPI', lw=1.5,range=range)
    bins_distil, _, _ = plt.hist(responsedistil, color="green",bins=xb, histtype='step', label=r'$E_\mathrm{Jet,\,reco}$'+f' DistillNet\nResolution: {r2:.4f}', lw=1.5,range=range)
    #bins_puppi, xb, _ = plt.hist(responsepuppi, bins=binsspace, histtype='step', label='Jets PUPPI', lw=1.5,range=range)
    figure.subplots_adjust(hspace=0)
    ...
    plt.vlines(np.mean(responsedistil), ymin=0, ymax=_ylim[1], color='green', alpha=0.99, linestyles="dotted", label=f'DistillNet mean\n response: {np.mean(responsedistil):.5f}')
    ...
    #resline(responseabc,"blue",f"ABC Net quantiles\nresolution: {r1:.4f}")
    #resline(responsedistil,"green",f"distil quantiles\nresolution: {r2:.4f}")
    #resline(responsepuppi,"orange",f"PUPPI quantiles\nresolution: {r3:.4f}")
    #resline(responsepuppi_true,"r",f"PUPPI true quantiles\nresolution: {r4:.4f}")
    plt.minorticks_on()
    ...
    plt.legend(fancybox=True, framealpha=0.8, loc='best', prop={'size': 16})#,bbox_to_anchor = (1.02,1),bbox_transform = plt.gca().transAxes,borderaxespad =0)
    if is_savefig:
        if is_corr:
...
def plot_jetenergy(abcjetE, puppijetE, distiljetE, genjetE, results_dir: str, results_dir_pdf: str, ptcut: int, is_savefig: bool = True):
    ...
    xminbin=0
    range=(xminbin,xmaxbin)
    bins_abc, xb, _ = plt.hist(np.clip(abcjetE,0,xmaxbin),bins=binsspace, histtype='step',label='Jet Energy GNN',lw=2,range=range)
    #bins_puppi, xb, _ = plt.hist(np.clip(puppijetE,0,xmaxbin), bins=binsspace, histtype='step', label='Jet Energy PUPPI', lw=2,range=range)
    bins_puppi, xb, _ = plt.hist(np.clip(puppijetE,0,xmaxbin), bins=binsspace, histtype='step', label='Jet Energy PUPPI true', lw=2,range=range)

    bins_distil, _, _ = plt.hist(np.clip(distiljetE,0,xmaxbin), bins=xb, histtype='step', label='Jet Energy distilNet', lw=2,range=range)
    #bins_swd, _, _ = plt.hist(np.clip(SWDjetE,0,xmaxbin), bins=xb,color="purple", histtype='step', label='Jet Energy SWDNet', lw=2,range=range)
    bins_gen ,_,_ = plt.hist(np.clip(genjetE,0,xmaxbin), bins=xb,color="black", histtype='step', label='Jet Energy gen', lw=1,range=range)
    #bins_gen2 ,_,_ = plt.hist(np.clip(genjetE_swd,0,xmaxbin), bins=xb,color="black", histtype='step', label='Jet Energy genswd', lw=2,range=range)
...
def plot_jetratio(abcjetE, puppijetE_true, distiljetE, results_dir: str, results_dir_pdf: str, ptcut: int, is_savefig: bool = True,is_corr:bool=False, is_ptcorr: bool = False):
    ...
    xminbin=0
    range=(xminbin,xmaxbin)
    bins_abc, xb, _ = ax[0].hist(np.clip(abcjetE,0,xmaxbin), bins=binsspace, histtype='step',label=r'$E_\mathrm{Jet,\,reco}\;$ GNN', range=range, lw=1.7)
    bins_puppi, _, _ = ax[0].hist(np.clip(puppijetE_true,0,xmaxbin), bins=xb, histtype='step', label=r'$E_\mathrm{Jet,\,reco}\;$ PUPPI',range=range, lw=1.4)
    #bins_NN, _, _ = plt.hist(genmet, bins=30, histtype='step',                         label='MET_gen', range=(0, 300), lw=1.5)
    bins_distil, _, _ = ax[0].hist(np.clip(distiljetE,0,xmaxbin), bins=xb, histtype='step', label=r'$E_\mathrm{Jet,\,reco}\;$ DistillNet',range=range, lw=1.7)

    #ax[0].set_xticks([])
    #ax[0].set_yticks([5000,10000,15000,20000])
    ax[1].set_yticks([0.8,1.2])
    figure.subplots_adjust(hspace=0)
    ax[0].set_xticks([])
    ...
    ax[1].errorbar(bincenter,ratio3,yerr=error3,fmt='.k',c='g',ecolor='g',capsize=5,ms=15,label="Ratio between distilNet and GNN jet energy")
    ax[1].errorbar(bincenter,ratio4,yerr=error4,fmt='^',c='tab:orange',ecolor='tab:orange',capsize=5,ms=6,label="Ratio between PUPPI and GNN jet energy")
...
def make_depositplots(nparticles, nstart, pu_dat, npu_dat, abcw, puppiw, predictions, plotdir, plotdir_pdf, is_savefig:bool =True, is_abc_puppisave: bool=False):
    ...
    img = ax.imshow(H_ptweighted_puppi, interpolation='nearest', origin='lower',
            extent=[yedges[0], yedges[-1], xedges[0], xedges[-1]], aspect='0.65')
    ...
    fig,ax = plt.subplots(figsize=[16,8])
    img = ax.imshow(H_ptweighted_model, interpolation='nearest', origin='lower',
            extent=[yedges[0], yedges[-1], xedges[0], xedges[-1]], aspect='0.65')
    cb = plt.colorbar(img,fraction=0.046,label='Particles')
    cb.ax.tick_params(labelsize=13)
    text = cb.ax.yaxis.label
    ...
    fig,ax = plt.subplots(figsize=[16,8])
    img = ax.imshow(H_ptweighted_distill, interpolation='nearest', origin='lower',
            extent=[yedges[0], yedges[-1], xedges[0], xedges[-1]], aspect='0.65')
    cb = plt.colorbar(img,fraction=0.046,label='Particles')
    cb.ax.tick_params(labelsize=13)
    text = cb.ax.yaxis.label
    ...
    fig,ax = plt.subplots(figsize=[16,8])
    img = ax.imshow(H_nopu, interpolation='nearest', origin='lower',
            extent=[yedges[0], yedges[-1], xedges[0], xedges[-1]], aspect='0.65')
    cb = plt.colorbar(img,fraction=0.046,label='Particles')
    cb.ax.tick_params(labelsize=13)
    text = cb.ax.yaxis.label
    ...
    fig,ax = plt.subplots(figsize=[16,8])
    img = ax.imshow(H_ptweighted_nopu, interpolation='nearest', origin='lower',
            extent=[yedges[0], yedges[-1], xedges[0], xedges[-1]], aspect='0.65')
    cb = plt.colorbar(img,fraction=0.046,label='Particles')
    cb.ax.tick_params(labelsize=13)
    text = cb.ax.yaxis.label
    ...
    fig,ax = plt.subplots(figsize=[16,8])
    img = ax.imshow(H_ptweighted_model/H_ptweighted_nopu,interpolation='nearest', origin='lower',
            extent=[yedges[0], yedges[-1], xedges[0], xedges[-1]], aspect='0.65',vmin = 0, vmax = 2,cmap=plt.get_cmap('RdBu'))
    ...
    fig,ax = plt.subplots(figsize=[16,8])
    img = ax.imshow(H_ptweighted_distill/H_ptweighted_nopu,interpolation='nearest', origin='lower',
            extent=[yedges[0], yedges[-1], xedges[0], xedges[-1]], aspect='0.65',vmin = 0, vmax = 2,cmap=plt.get_cmap('RdBu'))
    ...
    phibins = np.linspace(-3.1416,3.1416,nbins)
    fig,ax = plt.subplots(figsize=[16,8])
    img = ax.imshow(H_ptweighted_puppi/H_ptweighted_nopu,interpolation='nearest', origin='lower',
            extent=[yedges[0], yedges[-1], xedges[0], xedges[-1]], aspect='0.65',vmin = 0, vmax = 2,cmap=plt.get_cmap('RdBu'))
...
def derive_corrections(lower_bound: int, upper_bound: int, jetenergy_algorithm: list, jetenergy_gen: list, is_histcalc:bool=False, is_histoplots: bool = True):
...
def apply_jetcorrections(corrections_intervals: list, matched_algo, gen_matched_algo, is_prints: bool = False):
    ...
    for lower_bound, upper_bound in corrections_intervals:
        ...
        if is_prints:
            print(matched_algo)
        matched_algo = np.where((matched_algo_copy >= lower_bound) & (matched_algo_copy <= upper_bound), matched_algo / ratio, matched_algo)
        if is_prints:
            print(matched_algo)
...
def apply_jetcorrections_pt(corrections_intervals: list, matched_pt_algo: list, matched_pt_gen: list, matched_algo: list, gen_matched_algo: list, is_prints: bool = False):
    matched_pt_algo_copy = matched_pt_algo
    for lower_bound, upper_bound in corrections_intervals:
        ratio = derive_corrections_pt(lower_bound, upper_bound, matched_pt_algo, matched_pt_gen)
        if is_prints:
            print(matched_algo)
        matched_algo = np.where((matched_pt_algo_copy >= lower_bound) & (matched_pt_algo_copy <= upper_bound), matched_algo / ratio, matched_algo) #calc energy corrections
     #   matched_pt_algo = np.where((matched_pt_algo_copy >= lower_bound) & (matched_pt_algo_copy <= upper_bound), matched_pt_algo / ratio, matched_pt_algo) #calc pt corrections
        if is_prints:
            print(matched_algo)
    response_corrected = calcresponse(matched_algo, gen_matched_algo)
    return matched_algo, response_corrected#, matched_pt_algo   # then do response plot with matched_gen and matched_algo and see what happens
...
# distiljetE = corinputs(df_jetdata_model, 0)
# genjetE = corinputs(df_jetdata_model, 1)
# responsedistil = corinputs(df_jetdata_model, 2)
...
# responsepuppi = corinputs(df_jetdata_abc_puppi, 4)
    ...
    # genjetE = corinputs(df_jetdata_model,1)
    # responsedistil = corinputs(df_jetdata_model,2)
    # distilljetE_matched = corinputs(df_jetdata_model,3)
    # genjetE_distill_matched = corinputs(df_jetdata_model,4)
    ...
    plot_jetresolution(responseabc, responsedistil, responsepuppi, plotdir, plotdir_pdf,ptcut, is_savefig=is_savefig, is_corr=False)
                                                                                                            
    distilljetE_matched_corrected, response_distill_corrected = apply_jetcorrections(corr_intervals, distilljetE_matched, genjetE_distill_matched, is_prints=is_prints)
    abcjetE_matched_corrected, responseabc = apply_jetcorrections(corr_intervals, abcjetE_matched, genjetE_abc_matched, is_prints=is_prints)
    puppijetE_matched_corrected, responsepuppi = apply_jetcorrections(corr_intervals, puppijetE_matched, genjetE_puppi_matched, is_prints=is_prints)

    distilljetE_matched_ptcorrected, response_distill_ptcorr = apply_jetcorrections_pt(corr_intervals, distilljetpt_matched, genjetpt_distill_matched,
                                                                                        distilljetE_matched, genjetE_distill_matched, is_prints=is_prints)
    abcjetE_matched_ptcorrected, responseabc_ptcorr = apply_jetcorrections_pt(corr_intervals,abcjetpt_matched, genjetpt_abc_matched,
                                                                                abcjetE_matched, genjetE_abc_matched, is_prints=is_prints)
    puppijetE_matched_ptcorrected, responsepuppi_ptcorr = apply_jetcorrections_pt(corr_intervals,puppijetpt_matched, genjetpt_puppi_matched,
                                                                                    puppijetE_matched, genjetE_puppi_matched, is_prints=is_prints)

    plot_jetresolution(responseabc, response_distill_corrected, responsepuppi, plotdir, plotdir_pdf, ptcut, is_savefig=is_savefig, is_corr=True)
    plot_jetresolution(responseabc_ptcorr, response_distill_ptcorr, responsepuppi_ptcorr, plotdir, plotdir_pdf, ptcut, is_savefig=is_savefig, is_ptcorr=True)
    ...
    plot_jetratio(abcjetE_matched_corrected, puppijetE_matched_corrected, distilljetE_matched_corrected, plotdir, plotdir_pdf, ptcut, is_savefig=is_savefig,is_corr=True)
    plot_jetratio(abcjetE_matched_ptcorrected, puppijetE_matched_ptcorrected, distilljetE_matched_corrected, plotdir, plotdir_pdf, ptcut, is_savefig=is_savefig,is_ptcorr=True)
    # datadict_response_corrpt = {'distilljetE_matched_ptcorrected':distilljetE_matched_ptcorrected,
    #                             'response_distill_ptcorr':response_distill_ptcorr,
    #                             'abcjetE_matched_ptcorrected':abcjetE_matched_ptcorrected,
    #                             'responseabc_ptcorr':responseabc_ptcorr,
    #                             'puppijetE_matched_ptcorrected':puppijetE_matched_ptcorrected,
    #                             'responsepuppi_ptcorr':responsepuppi_ptcorr
    #                             }
    #datadict_response_corr = 
    return 
