...
as the clustering is highly resource intensive. Therefore, a range of i.e. 1000 events can be divided into 10 subintervals that can be then executed as 
10 parallel jobs as opposed to one big job with 1000 events. Afterwards, all subdataframes can then be combined into 
In previous versions the GNN was called ABCNet, therefore all the "abc" variables. 
"""

from jetclustering_helpers_cluster import JetEvent, resolution, make_jetenergyplot, make_jetresolutionplots, corinputs
import numpy as np
...
from distillnet_config import hparams, fl_inputs
...
parser.add_argument('--is_abc', action='store_true', help='Perform ABC and PUPPI clustering (default: True)')
parser.add_argument('--no-is_abc', dest='is_abc', action='store_false', help='Do not perform ABC and PUPPI clustering')
parser.set_defaults(is_abc=True)
parser.add_argument('--is_return_all', action='store_true', help='Perform all clustering -> GNN, Puppi and DistillNet(default: True)')
parser.add_argument('--no-is_return_all', dest='is_return_all', action='store_false', help='Do not perform all clustering')
...
savedir = join_and_makedir('./', 'Results_Jetclustering/')
filedir = './'
modelsavedir = './Models_Ensemble/'
...
print('Modelnumber ', modelnum)
print(saveinfo)

flist_inputs = [member.value for member in fl_inputs]

if is_abc_puppi:
    dir_saveinoabc = f'Jetclustering_ptcut{ptcut}_ptmatch/Abc_puppi__' + sample.replace('.h5','') + '/'
    results_dir = os.path.join(savedir, dir_saveinoabc)
    if not os.path.isdir(results_dir):
        os.makedirs(results_dir)
        
elif is_return_all:
    dir_saveinoabc = f'Jetclustering_ptcut{ptcut}_ptmatch/Allresults__' + saveinfo + '__'+ sample.replace('.h5','') + '/'
    results_dir = os.path.join(savedir, dir_saveinoabc)
    if not os.path.isdir(results_dir):
        os.makedirs(results_dir)
else:
    dir_saveinfo = f'Jetclustering_ptcut{ptcut}_ptmatch/' + saveinfo + '__' + sample.replace('.h5','') + '/'
