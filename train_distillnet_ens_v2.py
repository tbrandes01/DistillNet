...
def do_training_and_physicstest_DistillNet(filedir: str, savedir: str, device: str = 'cuda:0', run_number: int = 0, numtests: int = 0, wgt: float = 3, is_ensembletest: bool = False):
    """
    Train and evaluate Distillnet on given samples. Possible either once by executing main() in this script or as ensembletest, utilizing the run_distillensemble script.
    """
    ...
    model, criterion, optimizer, train_loader, test_loader, test, input_size = nn_setup(nn_inputdata, device, hparams['batch_size'],
                                                                                                        hparams['maketrain_particles'], hparams['train_split'],
                                                                                                        hparams['L1_hsize'],
    ...
    last_bin_ratio = make_weight_histogram(distill_wgts, abc_wgts, puppi_wgts, resolution_model, resolution_abc,
