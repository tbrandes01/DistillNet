...
def Jetmatching(dRcrit, pslistjet1, pslistjet2, JetE1, JetE2, Jetpt1, Jetpt2):  # match 2 Jets based on dR cirteria
    """
...
def JetEvent(
    ...
    with h5py.File(filename, "r") as f:
        ...
        del features_reshaped, abc_weights_reshaped, puppi_weights_reshaped, abc_vec_reshaped, puppi_vec_reshaped, gen_vec_reshaped, default_vec_reshaped
        gc.collect()
        ...
        if is_return_all:
            ...
            Jetgen = Jetcalc(gen_vec_np, np.ones_like(gen_vec_np[:, 2]), ptcut) #ones like to get a weight vector the same size as the gen vector pt

            abcmatchjets, genabcmatchjets, abcE, genEabc, abc_pt_match, gen_pt_abcmatch = Jetmatching(
            dR, JetAbc[0], Jetgen[0], JetAbc[1], Jetgen[1], JetAbc[2], Jetgen[2]
            )
            puppimatchjets, genpuppimatchjets, puppiE, genEpuppi, puppi_pt_match, gen_pt_puppimatch = Jetmatching(
                dR, Jetpuppi[0], Jetgen[0], Jetpuppi[1], Jetgen[1], Jetpuppi[2], Jetgen[2]
            )
            distilmatchjets, gendistilmatchjets, distilE, genEdistil, distill_pt_match, gen_pt_distillmatch = Jetmatching(
            ...
            return (JetAbc, Jetpuppi, Jetgen, responseabc, responsepuppi, abcE, genEabc, puppiE, genEpuppi,
                    Jetdistill, Jetgen, responsedistil, distilE, genEdistil,
                    abc_pt_match, gen_pt_abcmatch, puppi_pt_match, gen_pt_puppimatch,
                    distill_pt_match, gen_pt_distillmatch)
        
        else:
            if is_abc_puppi:
                JetAbc = Jetcalc(default_vec_np, np.concatenate(abcw_np), ptcut)  # jet clustering for ABCNet and Puppi
                Jetpuppi = Jetcalc(default_vec_np, np.concatenate(puppiw_np), ptcut)
            else:
                Jetdistill = Jetcalc(default_vec_np, predictions, ptcut) # only jet clustering dor DistillNet
                del default_vec_np, predictions
                gc.collect()

            Jetgen = Jetcalc(gen_vec_np, np.ones_like(gen_vec_np[:, 2]), ptcut) #gen has to be clustered either way
            ...
            if is_abc_puppi: # only return ABC and Puppi jets
                ...
                responsedistil = rescalc(distilE, genEdistil)
                return Jetdistill, Jetgen, responsedistil, distilE, genEdistil, distill_pt_match, gen_pt_distillmatch
...
def Jetcalc(features, weights, ptcut):
    ...
    return Esortedjets, energy, ptrans  # returns list of pseudojets sorted by energy, list of energies and list of pt's from sorted energies
